...
export default function TimerDisplay({ seconds }) {
    return (
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
            <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '10px', width: '350px', height: '100px', fontSize: '73px'}}>
                {secondsToTime(seconds).h} : {secondsToTime(seconds).m} : {secondsToTime(seconds).s}
            </div>
        </div>
    );
}